...
def convert_csv_to_json():
    ...
    for _, row in df.iterrows():
        shelter = {
            ...
            "name": str(row['施設・場所名']),
            "address": str(row['住所']),
            "generalFlooding": row['洪水'] == 1,
            "landslide": row['崖崩れ、土石流及び地滑り'] == 1,
            "highTide": row['高潮'] == 1,
            "earthquake": row['地震'] == 1,
            "tsunami": row['津波'] == 1,
            "fire": row['大規模な火事'] == 1,
            "internalFlooding": row['内水氾濫'] == 1,
            "volcano": row['火山現象'] == 1,
            "isSameAsEvacuationCenter": row['指定避難所との住所同一'] == 1,
            "latitude": float(row['緯度']),
            "longitude": float(row['経度']),
            "additionalInfo": str(row['備考']) if pd.notna(row['備考']) else ""
        }
